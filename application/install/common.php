...
function write_config($config, $auth)
{
    if (is_array($config)) {
        // 读取配置内容
        $conf = file_get_contents('../application/install/data/database.tpl');
        ...
        // 写入应用配置文件
        if (!IS_WRITE) {
            return '由于您的环境不可写，请复制下面的配置文件内容覆盖到相关的配置文件，然后再登录后台。<p>/config/database.php</p>
            <textarea name="" style="width:650px;height:185px">' . $conf . '</textarea>';
        } else {
            if (file_put_contents('../config/database.php', $conf)) {
                show_msg('配置文件写入成功');
            } else {
...
{
    // 读取SQL文件
    $sql = file_get_contents('../application/install/data/install.sql');
    $sql = str_replace("\r", "\n", $sql);
    $sql = explode(";\n", $sql);
...
function register_administrator($db, $prefix, $admin, $auth)
{
    ...
    $sql="INSERT INTO `[PREFIX]user` (`roles`, `account`, `nickname`, `password`, `bind_account`, `last_login_time`, `last_login_ip`, `login_count`, `verify`, `mobile`, `email`, `remark`, `sign_img`, `create_time`, `update_time`, `status`) VALUES ('1', '[NAME]', '超级管理员', '[PASS]', '', '[TIME]', '[IP]', '0', '', '', '', '', '', '0', '0', '1')";
...
function update_tables($db, $prefix = '')
{
    ...
    // 替换表前缀
    $sql = str_replace(" `pg_", " `{$prefix}", $sql);
