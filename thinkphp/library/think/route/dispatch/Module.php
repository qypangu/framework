    ...
    public function init()
    {
        ...
        if ($this->rule->getConfig('app_multi_module')) {
            ...
            if ($bind && preg_match('/^[a-z]/is', $bind)) {
                ...
                $available = true;
            }elseif (is_dir($this->app->getAppPath().Loader::parseName($module, 1,false))){ //hjwtp2005@qq.com 修正模块是驼峰法是userAdmin的url转换为user_admin的情况
                $module=Loader::parseName($module, 1,false);
                $available=true;
            }
		...
		$this->actionName=Loader::parseName($this->actionName, 1,false); //这个方法可以把操作设置为和操制器一样EditPassword的方式
        // 设置当前请求的控制器、操作
        $this->request
